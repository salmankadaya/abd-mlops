...
# MAGIC 
# MAGIC ## Instructions:
# MAGIC 1. Run the notebook against a cluster with Databricks ML Runtime version 10.4.x-cpu, to best re-create the training environment.
...
# MAGIC Run the notebook against a cluster with Databricks ML Runtime version 10.4.x-cpu, to best re-create the training environment.. The cell below downloads the model artifacts associated with your model in the remote registry, which include `conda.yaml` and `requirements.txt` files. In this notebook, `pip` is used to reinstall dependencies by default.
# MAGIC 
...
model_name = "iris-classifier"
input_table_name = "default.iris_pred_csv"
output_table_path = "/FileStore/batch-inference/iris-classifier"
...
# MAGIC ```python
# MAGIC output_table = "" # Example: "ml.batch-inference.iris-classifier"
